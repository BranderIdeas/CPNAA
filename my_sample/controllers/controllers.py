    ...
    def validar_tramites(self, **kw):
        ...
        if self.validar_captcha(kw.get('token')):
            if origen == 'renovacion':
                ...
                if (dias < 0):
                    #return { 'ok': 'True'}
                    return { 'ok': 'False', 'messaje': 'Su solicitud de Renovación de Licencia Temporal Especial no puede ser recibida, tiempo vencido para radicar la solicitud. Favor tramitar Licencia Temporal Especial por primera vez en el link: https://cpnaa.gov.co/tramite-licencia-temporal/ -- fecha expiracion' + str(fecha_expiracion) }
                else:
