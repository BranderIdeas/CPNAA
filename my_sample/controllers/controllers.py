    ...
    def validar_tramites(self, **kw):
        ...
        if self.validar_captcha(kw.get('token')):
            if origen == 'renovacion':
                ...
                if (dias < 0):
                    return { 'ok': 'True'}
                    #return { 'ok': 'False', 'messaje': 'true: va por dias < 0: try: dias' + str(dias) + ' fecha expiracion: ' + str(fecha_expiracion) + ' now: ' + str(now) + '/' + str(timedelta(days=1)) + ' except: dias' + str(dias) + ' fecha expiracion: ' + str(fecha_expiracion) + ' ano: ' + str(ano) + ' now: ' + str(now) + '/' + str(timedelta(days=1)) }
                else:
