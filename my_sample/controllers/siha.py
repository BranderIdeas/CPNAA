...
class Siha(http.Controller):
    @http.route('/siha', auth='user', website=True)
    def siha(self):
        _logger.info(http.request.session)
        if http.request.session.uid == None:
            return http.request.redirect('/web/login')
        else:
            return http.request.render('my_sample.siha')
        
        
    @http.route('/siha/<path:dummy>', auth='user', website=True)
    def siha_redirect(self):
        _logger.info(http.request.session)
        if http.request.session.uid == None:
            return http.request.redirect('/web/login')
        else:
    ...
    @http.route('/siha_api/data_profesional', methods=['POST'], type="json", auth='user', website=True, cors='*')
    ...
    @http.route('/siha_api/get', methods=['POST'], type="json", auth='user', website=True)
    ...
    def get_project_usages_by_simulation_id(self, simulation_id):
        ...
        return http.request.env[modelo].sudo().search_read(domain, fields)
    ...
    @http.route('/siha_api/create', methods=['POST'], type="json", auth='user', website=True)
    ...
    @http.route('/siha_api/update', methods=['POST'], type="json", auth='user', website=True)
    def update_simulation(self, **kw):
        ...
        if record:
            complementary_fees = simulation.get('x_complementary_fees_ids', False)
            construction_service = simulation.get('x_construction_service_ids', False)
            if complementary_fees and len(complementary_fees) == 0:
                record.x_complementary_fees_ids = [(5, 0, 0)] # Elimina todos los registros existentes en el campo Many2Many
            if construction_service and len(construction_service) == 0:
    ...
    @http.route('/siha_api/delete', methods=['POST'], type="json", auth='user', website=True)
    ...
    @http.route('/siha_api/save_uses', methods=['POST'], type="json", auth='user', website=True)
    ...
    @http.route('/siha_api/delete_uses', methods=['POST'], type="json", auth='user', website=True)
    ...
    @http.route('/siha_api/data_select/<string:modelo>', methods=['POST'], type="json", auth='user', website=True)
    ...
    @http.route('/siha_api/getSMLV', methods=['POST'], type="json", auth='user', website=True)
    def getSMLV(self):
        _logger.info(http.request.session.login)
